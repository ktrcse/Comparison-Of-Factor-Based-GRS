...
u_df = ratings_df.pivot(index = 'user_id', columns ='movie_id', values = 'rating').fillna(0)
R_m=np.matrix(u_df)
...
for N in range(2,12):
        ...
        for i in range (1,5):
                ...
                #print('group size...')
                #print(N)
                u1=np.random.choice(users['user_id'].unique(),1)
                i=u1-1
                vals=[]
                vals.append(u1.item(0))
                c=1
                while(c!=N):
                    u2=np.random.choice(users['user_id'].unique(),1)
                    j=u2-1
                    s=0
                    for k in range(0,R_m.shape[1]):
                            s=s+(R_m[i].item(k)*R_m[j].item(k))
                    left=0
                    right=0
                    for k in range(0,R_m.shape[1]):
                            if(R_m[i].item(k)>0 and R_m[j].item(k)>0):
                                    left=left+(R_m[i].item(k)*R_m[i].item(k))
                                    right=right+(R_m[j].item(k)*R_m[j].item(k))
                    l=np.sqrt(left).item(0)
                    r=np.sqrt(right).item(0)
                    if(l*r >0):
                            d=(s)/(l*r)
                    else:
                            d=0
                    if(d>.95):
                            vals.append(u2.item(0))
                            v=np.unique(vals)
                            c=len(v)
                ...
                G_df=ratings_df.set_index('user_id').loc[v].reset_index()
                ...
                prec1.append(float(tp1)/20)
                recall1.append(float(tp1)/c)
                prec2.append(float(tp2)/20)
                recall2.append(float(tp2)/c)
                prec3.append(float(tp3)/20)
                recall3.append(float(tp3)/c)
...
plt.title('user-sim based group - Pred_Error')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
#plt.show()
...
plt.plot(x,y2,label='BF')
plt.plot(x,y3,label='WBF')
plt.title('user-sim based group -Precision')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
#plt.show()
...
plt.plot(x,y2,label='BF')
plt.plot(x,y3,label='WBF')
plt.title('user-sim based group -Recal')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
plt.show()
