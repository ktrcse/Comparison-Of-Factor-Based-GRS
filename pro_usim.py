...
for N in range(2,13):
        ...
        for i in range (1,11):
                ...
                #print('group size...')
                #print(N)
                v=np.random.choice(users['age'].unique(),1,replace=False)
                if(v>=5 and v<=25):
                        vals=np.random.choice(m1,N)    
                elif(v>=26 and v<=50):
                        vals=np.random.choice(m2,N)    
                else:
                        vals=np.random.choice(m3,N)    

                u=users.set_index('age').loc[vals].reset_index()
                au=np.random.choice(u['user_id'].unique(), N, replace=False)
                G_df=ratings_df.set_index('user_id').loc[au].reset_index()
...
plt.title('user-age based group - Pred_Error')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
#plt.show()
...
plt.plot(x,y2,label='BF')
plt.plot(x,y3,label='WBF')
plt.title('user-age based group -Precision')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
#plt.show()
...
plt.plot(x,y2,label='BF')
plt.plot(x,y3,label='WBF')
plt.title('user-age based group -Recal')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
plt.show()
