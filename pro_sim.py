...
for N in range(2,13):
        ...
        for i in range (1,11):
                ...
                #print('group size...')
                #print(N)
                oc=np.random.choice(users['occupation'].unique(),1,replace=False)
                u=users.set_index('occupation').loc[oc].reset_index()
                while(len(u)<N):
                        oc=np.random.choice(users['occupation'].unique(),1,replace=False)
                        u=users.set_index('occupation').loc[oc].reset_index()
                
                vals=np.random.choice(u['user_id'].unique(), N, replace=False)
...
c1=np.column_stack((np.transpose(M_mean_accur_AF),np.transpose(M_mean_accur_BF)))
c2=np.column_stack(((c1),np.transpose(M_mean_accur_WBF)))
c3=np.column_stack(((c2),np.transpose(M_min_accur)))
...
plt.title('occupation based group - Pred_Error')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
#plt.show()
...
plt.plot(x,y2,label='BF')
plt.plot(x,y3,label='WBF')
plt.title('occupation based group -Precision')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
#plt.show()
...
plt.plot(x,y2,label='BF')
plt.plot(x,y3,label='WBF')
plt.title('occupation based group -Recal')
plt.xlabel('Group')
plt.grid(True)
plt.legend()
plt.show()
